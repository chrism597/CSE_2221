...
public final class NaturalNumberStaticOps {
     ...
     * @ensures [displays menu, returns user command]
     */
    ...
    private static void decrement(NaturalNumber n) {
        ...
        int digit = n.divideBy10(); // remove last digit

        if (digit == 0) {
            // Need to borrow from a higher digit
            decrement(n);
            digit = 9;
        } else {
            digit -= 1;
        ...
        n.multiplyBy10(digit); // rebuild number
    }
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        if (n.compareTo(ONE_THOUSAND) < 0) {
            ...
            printWithCommas(n, out); // recursive call on quotient
            out.print(",");
            out.print(String.format("%03d", remainder.toInt())); // pad remainder with zeros
     ...
     */
    static String toStringWithCommas(NaturalNumber n) {
        assert n != null : "Violation of: n is not null";
        ...
        if (n.compareTo(ONE_THOUSAND) < 0) {
            ...
            // divide n by 1000
            NaturalNumber remainder = n.divide(ONE_THOUSAND);
            String part = String.format("%03d", remainder.toInt());
            String result = toStringWithCommas(n) + "," + part;
