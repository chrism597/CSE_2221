...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        // TODO: fill in body
        // check if str is not a substring of any strings already in strSet
        boolean notSubstringInSet = true;
        for (String s : strSet) {
            if (s.contains(str)) {
                notSubstringInSet = false;
            }
        }

        // if str is not a substring of any strings already in strSet,
        // remove its substrings from strSet and add it
        if (notSubstringInSet) {
            Set<String> substringSet = new Set1L<>();
            for (String s : strSet) {
                if (str.contains(s)) {
                    substringSet.add(s);
                }
            ...
            // remove all substrings of str from strSet
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // TODO: fill in body
        // initialize the set for the result
